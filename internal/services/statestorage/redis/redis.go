...
func (r redisStateStorage) GetRoom(ctx context.Context, scheduler, roomID string) (*entities.GameRoom, error) {
	...
	if err != nil {
		if err == redis.Nil {
			return nil, statestorage.NewRoomNotFoundError(scheduler, roomID)
		}
		return nil, statestorage.WrapError("error storing room on redis", err)
	}
	...
	if err != nil {
		return nil, statestorage.WrapError("error unmarshalling json", err)
	}
...
func (r *redisStateStorage) CreateRoom(ctx context.Context, room *entities.GameRoom) error {
	...
	_, err = p.Exec()
	if err != nil {
		return statestorage.WrapError("error storing room on redis", err)
	}

	if !roomCmd.Val() || statusCmd.Val() < 1 || pingCmd.Val() < 1 {
		return statestorage.NewRoomAlreadyExistsError(room.Scheduler, room.ID)
	}
...
func (r *redisStateStorage) UpdateRoom(ctx context.Context, room *entities.GameRoom) error {
	...
	p.ZAddXXCh(getRoomStatusSetRedisKey(room.Scheduler), redis.Z{
		Member: room.ID,
		Score:  float64(room.Status),
	})
	p.ZAddXXCh(getRoomPingRedisKey(room.Scheduler), redis.Z{
		Member: room.ID,
		Score:  float64(room.LastPingAt.Unix()),
	})
	...
	_, err = p.Exec()
	if err != nil {
		return statestorage.WrapError("error updating room on redis", err)
	}

	if !roomCmd.Val() {
		return statestorage.NewRoomNotFoundError(room.Scheduler, room.ID)
	}
...
func (r *redisStateStorage) RemoveRoom(ctx context.Context, scheduler, roomID string) error {
	...
	cmders, err := p.Exec()
	if err != nil {
		return statestorage.WrapError("error removing room from redis", err)
	}
	for _, cmder := range cmders {
		cmd := cmder.(*redis.IntCmd)
		if cmd.Val() == 0 {
			return statestorage.NewRoomNotFoundError(scheduler, roomID)
		}
	}
	return nil
}
...
func (r *redisStateStorage) GetAllRoomIDs(ctx context.Context, scheduler string) ([]string, error) {
	...
	if err != nil {
		return nil, statestorage.WrapError("error listing rooms on redis", err)
	}
	return rooms, nil
}
...
func (r *redisStateStorage) GetRoomCount(ctx context.Context, scheduler string) (int, error) {
	...
	if err != nil {
		return 0, statestorage.WrapError("error counting rooms on redis", err)
	}
