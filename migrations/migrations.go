package migrations

import (
	"bytes"
	"compress/gzip"
	"fmt"
	"io"
	"strings"
)

func bindata_read(data []byte, name string) ([]byte, error) {
	gz, err := gzip.NewReader(bytes.NewBuffer(data))
	if err != nil {
		return nil, fmt.Errorf("Read %q: %v", name, err)
	}

	var buf bytes.Buffer
	_, err = io.Copy(&buf, gz)
	gz.Close()

	if err != nil {
		return nil, fmt.Errorf("Read %q: %v", name, err)
	}

	return buf.Bytes(), nil
}

var __0001_createschedulertable_sql = []byte("")

func _0001_createschedulertable_sql() ([]byte, error) {
	return bindata_read(
		__0001_createschedulertable_sql,
		"0001-CreateSchedulerTable.sql",
	)
}

var __0002_createusertable_sql = []byte("")

func _0002_createusertable_sql() ([]byte, error) {
	return bindata_read(
		__0002_createusertable_sql,
		"0002-CreateUserTable.sql",
	)
}

var __0003_createschedulerversionstable_sql = []byte("")

func _0003_createschedulerversionstable_sql() ([]byte, error) {
	return bindata_read(
		__0003_createschedulerversionstable_sql,
		"0003-CreateSchedulerVersionsTable.sql",
	)
}

var __0004_alterschedulertableaddversion_sql = []byte("")

func _0004_alterschedulertableaddversion_sql() ([]byte, error) {
	return bindata_read(
		__0004_alterschedulertableaddversion_sql,
		"0004-AlterSchedulerTableAddVersion.sql",
	)
}

var __0005_alterversiontype_sql = []byte("")

func _0005_alterversiontype_sql() ([]byte, error) {
	return bindata_read(
		__0005_alterversiontype_sql,
		"0005-AlterVersionType.sql",
	)
}

var __0006_addstatuscolumntoversion_sql = []byte("")

func _0006_addstatuscolumntoversion_sql() ([]byte, error) {
	return bindata_read(
		__0006_addstatuscolumntoversion_sql,
		"0006-AddStatusColumnToVersion.sql",
	)
}

var __0007_addrollbackversioncolumntoversion_sql = []byte("")

func _0007_addrollbackversioncolumntoversion_sql() ([]byte, error) {
	return bindata_read(
		__0007_addrollbackversioncolumntoversion_sql,
		"0007-AddRollbackVersionColumnToVersion.sql",
	)
}

// Asset loads and returns the asset for the given name.
// It returns an error if the asset could not be found or
// could not be loaded.
func Asset(name string) ([]byte, error) {
	cannonicalName := strings.Replace(name, "\\", "/", -1)
	if f, ok := _bindata[cannonicalName]; ok {
		return f()
	}
	return nil, fmt.Errorf("Asset %s not found", name)
}

// AssetNames returns the names of the assets.
func AssetNames() []string {
	names := make([]string, 0, len(_bindata))
	for name := range _bindata {
		names = append(names, name)
	}
	return names
}

// _bindata is a table, holding each asset generator, mapped to its name.
var _bindata = map[string]func() ([]byte, error){
	"0001-CreateSchedulerTable.sql": _0001_createschedulertable_sql,
	"0002-CreateUserTable.sql": _0002_createusertable_sql,
	"0003-CreateSchedulerVersionsTable.sql": _0003_createschedulerversionstable_sql,
	"0004-AlterSchedulerTableAddVersion.sql": _0004_alterschedulertableaddversion_sql,
	"0005-AlterVersionType.sql": _0005_alterversiontype_sql,
	"0006-AddStatusColumnToVersion.sql": _0006_addstatuscolumntoversion_sql,
	"0007-AddRollbackVersionColumnToVersion.sql": _0007_addrollbackversioncolumntoversion_sql,
}
// AssetDir returns the file names below a certain
// directory embedded in the file by go-bindata.
// For example if you run go-bindata on data/... and data contains the
// following hierarchy:
//     data/
//       foo.txt
//       img/
//         a.png
//         b.png
// then AssetDir("data") would return []string{"foo.txt", "img"}
// AssetDir("data/img") would return []string{"a.png", "b.png"}
// AssetDir("foo.txt") and AssetDir("notexist") would return an error
// AssetDir("") will return []string{"data"}.
func AssetDir(name string) ([]string, error) {
	node := _bintree
	if len(name) != 0 {
		cannonicalName := strings.Replace(name, "\\", "/", -1)
		pathList := strings.Split(cannonicalName, "/")
		for _, p := range pathList {
			node = node.Children[p]
			if node == nil {
				return nil, fmt.Errorf("Asset %s not found", name)
			}
		}
	}
	if node.Func != nil {
		return nil, fmt.Errorf("Asset %s not found", name)
	}
	rv := make([]string, 0, len(node.Children))
	for name := range node.Children {
		rv = append(rv, name)
	}
	return rv, nil
}

type _bintree_t struct {
	Func func() ([]byte, error)
	Children map[string]*_bintree_t
}
var _bintree = &_bintree_t{nil, map[string]*_bintree_t{
	"0001-CreateSchedulerTable.sql": &_bintree_t{_0001_createschedulertable_sql, map[string]*_bintree_t{
	}},
	"0002-CreateUserTable.sql": &_bintree_t{_0002_createusertable_sql, map[string]*_bintree_t{
	}},
	"0003-CreateSchedulerVersionsTable.sql": &_bintree_t{_0003_createschedulerversionstable_sql, map[string]*_bintree_t{
	}},
	"0004-AlterSchedulerTableAddVersion.sql": &_bintree_t{_0004_alterschedulertableaddversion_sql, map[string]*_bintree_t{
	}},
	"0005-AlterVersionType.sql": &_bintree_t{_0005_alterversiontype_sql, map[string]*_bintree_t{
	}},
	"0006-AddStatusColumnToVersion.sql": &_bintree_t{_0006_addstatuscolumntoversion_sql, map[string]*_bintree_t{
	}},
	"0007-AddRollbackVersionColumnToVersion.sql": &_bintree_t{_0007_addrollbackversioncolumntoversion_sql, map[string]*_bintree_t{
	}},
}}
